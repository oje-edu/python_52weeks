...
# NOTE: disable insecure HTTPS request warnings that NAPALM gets
import urllib3
...
# make a copy of NXOS so that we can do both SSH and NXAPI connections
...
for device_type, device in devices.items():
    ...
    print(f"----- connect to               device {device['hostname']}, type_ {device_type} ---------")
    napalm_device.open()
